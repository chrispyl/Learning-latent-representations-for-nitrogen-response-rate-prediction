...
import helper_functions, location_match, schemas
...
def combine_input_datasets(WeatherDataAllDF, simulationsDF, biophysicals):
    ...
    simulationsDF = simulationsDF.select(*['File', 'FertiliserApplied', 'HerbageCut'],
                                        *biophysicals,
                                        F.to_date(F.col('Date'), 'dd/MMM/yyyy').alias('Date'),
...
def extract_columns_from_filename(dataDF):
    ...
    dataDF = dataDF.withColumn('parts', F.split(F.col('File'), '_'))\
                   ...
                   .select(*dataDF.columns,
                            F.substring(F.element_at('parts', 1), 2, 4).cast(IntegerType()).alias('Year'),
                            F.substring(F.element_at('parts', 2), 2, 2).cast(IntegerType()).alias('FertMonth'),
                            F.substring(F.element_at('parts', 3), 2, 2).cast(IntegerType()).alias('FertDay'),
                            F.substring(F.element_at('parts', 4), 11, 3).cast(IntegerType()).alias('FertRate'),
                            F.substring(F.element_at('parts', 5), 5, 3).cast(IntegerType()).alias('SoilWater'),
                            F.when(F.col('SoilFertility') == 'Low', 1).otherwise(F.when(F.col('SoilFertility') == 'Med', 2).otherwise(3)).alias('SoilFertility'),
    ...
    baseline_herbagecut_aggDF = dataDF.filter(F.col('FertRate') == 0)\
                                      ...
                                      .agg(F.sum(
                                                F.when(
                                                    ...
                                                    (F.col('DaysRelative') >= 0) & (F.col('DaysRelative') <= 65),
                                                    F.col('HerbageCut'))).alias('baseline_0_65_HerbageCut'))
    
    dataDF = dataDF.join(baseline_herbagecut_aggDF, ['Weather', 'Year', 'SoilWater', 'SoilFertility', 'Irrigation', 'FertMonth', 'FertDay'])
...
def create_aggregated_dataset(dataDF, timeframes, biophysicals, avg_weather, avg_biophysical, downlimit, uplimit):
    ...
    weather_factors = ['Rain', 'MaxT', 'MinT', 'RH', 'VP', 'Wind', 'Radn', 'PET']

    average_weather = [avg_per_days(timeframe, total_days, factor) for timeframe in timeframes for factor in weather_factors]
    average_biophysical = [avg_per_days(timeframe, total_days, factor) for timeframe in timeframes for factor in biophysicals]
...
def preprocessing_pipeline(remove_herbagecut, timeframes, downlimit, uplimit, biophysicals, avg_weather, avg_biophysical, simulations_path, weather_path, nitrogen_path, base_path):
    ...
    #read data
    simulationsDF = spark.read.csv(simulations_path, header=True, mode='FAILFAST', schema=schemas.clover_simulations_schema)
    WeatherDataAllDF = spark.read.csv(weather_path, header=True, mode='FAILFAST', schema=schemas.weather_schema)
    NitrogenResponseRateDF = spark.read.csv(nitrogen_path, header=True, mode='FAILFAST', schema=schemas.nitrogen_response_rate_schema)

    NitrogenResponseRateDF = NitrogenResponseRateDF.filter(F.col('PastureType') == 'GrassClover') #keep only the values that refer to clover
    
    #setting up data before aggregations
    dataDF = combine_input_datasets(WeatherDataAllDF, simulationsDF, biophysicals)
    ...
    for df_name, df in {'train': train_dataDF, 'test': test_dataDF}.items():
        ...
        featureDF = create_aggregated_dataset(df, timeframes, biophysicals, avg_weather, avg_biophysical, downlimit, uplimit)
        
        if df_name == 'train':
            for location_type in ['known', 'unknown']:
                    for location in ['Clim2']:
                        for model_type in ['local']:
                            if location_type == 'known':
                                #if the location is known, use locations from climate matching
                                locations = location_match.climate_match(model_type, location_type, location)
                                df = featureDF.filter(F.col('Weather').isin(locations))
                            else:
                                #if the location is unknown, use the nearest locations
                                if model_type == 'local':
                                    closest_locations = location_match.get_n_closest_locations(1)[location]
                                elif model_type == 'regional':
                                    closest_locations = location_match.get_n_closest_locations(3)[location]
                                else:
                                    closest_locations = location_match.get_n_closest_locations(7)[location]
                                df = featureDF.filter(F.col('Weather').isin(closest_locations))    
                            
                            #create the path where the test sets will be saved
                            save_path = base_path + '/' + model_type + '/' + location_type + '/' + df_name + '/'
                            helper_functions.create_path(save_path)
                            #save results
                            df.write.csv(save_path + location + '.csv', header = True)
                            print('Finished: ' + model_type + ',' + location_type + ',' + location)
        else:           
            #create the path where the test sets will be saved
