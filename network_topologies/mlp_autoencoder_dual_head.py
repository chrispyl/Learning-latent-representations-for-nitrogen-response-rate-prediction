...
class DualHeadAutoencoder(T.nn.Module):
    ...
    def forward(self, x):
        ...
        nrr_out = self.nrr_regressor(enc_out)
...
def validate(net, dataloader, loss):
    ...
    with T.no_grad():    
            ...
            for i, (x, y_nrr, rest) in enumerate(dataloader):
                ...
                dec_out, nrr_out = net(x)
            
                l = loss(dec_out, x) + loss(nrr_out, y_nrr.unsqueeze(1))
...
def test(net, dataloader):
    ...
    with T.no_grad():
        ...
        for i, (x, y_nrr, rest) in enumerate(dataloader):
            ...
            #prediction = net(x)
            dec_pred, nrr_pred = net(x)          
            
            squared_error_dec = (dec_pred-x)**2
...
def run(scenario_id, batch_size, latent_dims, autoencoder_epochs, patience, train_clims, validation_clims, test_clim, metamodel_type, test_type, scenario_folder, filename_train, filename_validate, filename_test, irrigation, base_path, save_path):
    '''Returns None
    ...
    autoencoder_epochs: int, how many epochs to train
    ...
    #initialize the net
    net = DualHeadAutoencoder(latent_dims)
    net.to(device)
    
    #train the net
    net, avg_train_losses, avg_valid_losses = train(net, train_dataloader, validation_dataloader, autoencoder_epochs, patience, tensorboard_writer)
    T.save(net.encoder.state_dict(), 'saved_encoder.pt')
    
    checkpointed_net = DualHeadAutoencoder(latent_dims)
    checkpointed_net.load_state_dict(T.load('model_checkpoint.pt'))
    
    #test the fully trained net
    training_error_dec, training_error_nrr, trraining_predictions_df = test(checkpointed_net, train_dataloader)
    s_f_tr = scenario_id + ', ' + 'Fully trained autoencoder net train rmse:' + str(training_error__dec) + ' ' + str(training_error_nrr)
    print(s_f_tr)
    
    testing_error_dec, testing_error_nrr, testing_predictions_df  = test(checkpointed_net, test_dataloader)
    s_f_te = scenario_id + ', ' + 'Fully trained autoencoder net test rmse:' + str(testing_error_dec) + ' ' + str(testing_error_nrr)
    print(s_f_te)
            ...
            irrigation in [[['Clim1'], ['Clim1'], 'Clim1', 'local', 'known', \
                ...
                for latent_dims in [120]:
                    run(scenario_id = 'seed_' + str(seed),
                        ...
                        base_path = base_path,
                        save_path = base_path + 'predictions/' + test_clim + '/' + scenario_folder + '/res_autoencoder_multitask_adamw_fert_soilwater/' + test_type + '/' + metamodel_type + '/')
